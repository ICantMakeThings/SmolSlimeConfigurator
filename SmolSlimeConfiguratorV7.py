...
# Verify checksum if provided in URL (optional: implement SHA256 lookup table if needed)
            checksum_url = firmware_url + ".sha256"  # assuming a .sha256 file exists
            try:
                r = requests.get(checksum_url, timeout=5)
                r.raise_for_status()
                expected_checksum = r.text.strip().split()[0]
                ...
                if actual_checksum != expected_checksum:
                    append_text(f"[Error] Checksum mismatch! {actual_checksum} != {expected_checksum}\n", "error")
                    return
                else:
                    append_text("Checksum verified!\n", "success")
            except Exception:
                append_text("No checksum verification available.\n", "error")
